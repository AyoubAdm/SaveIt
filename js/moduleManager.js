class ModuleManager {
  ...
  async loadModule(moduleName) {
    ...
    moduleData.geometry.forEach((geomData) => {
        ...
        boxMaterial.diffuseTexture.wAng = Math.PI / 2;
       
        //rotation de la texture a 90°
    ...
    moduleData.binSpawns.forEach((binData) => {
      ...
      bin.position = new BABYLON.Vector3(
        this.randomXPosition(),
        binData.position.y,
        binData.position.z + decalage
      );
      this.addCollisionDetection(bin);
    });

    // Créez les arbres à partir des données JSON
    if (moduleData.treeSpawns) {
        moduleData.treeSpawns.forEach((treeData) => {
          const tree = this.createTree();
          tree.position = new BABYLON.Vector3(
            treeData.position.x + 10,  // Déplace l'arbre à droite de la route
            treeData.position.y,
            treeData.position.z + decalage
          );
          const tree2 = this.createTree();
          tree2.position = new BABYLON.Vector3(
            treeData.position.x - 10,  // Déplace l'arbre à gauche de la route
            treeData.position.y,
            treeData.position.z + decalage
          );
        });
  ...
  createWaste(type) {
    ...
    //Fonction pour créer les arbr

  createTree() {
    // Ici, j'utilise un cylindre pour représenter un arbre, mais vous pouvez utiliser tout autre forme ou un modèle 3D pour un arbre
    const tree = BABYLON.MeshBuilder.CreateCylinder("tree", { height: 5, diameter: 1 }, this.scene);
    tree.material = new BABYLON.StandardMaterial("treeMaterial", this.scene);
    tree.material.diffuseColor = new BABYLON.Color3(0, 1, 0); // Vert pour l'arbre
    return tree;
  } 
    ...
    const ground = BABYLON.MeshBuilder.CreateGroundFromHeightMap(
        ...
        "textures/bPuyw.png", // Remplacez par le chemin de votre heightmap
        {
            ...
            onReady: () => {
                ...
                groundMaterial.diffuseTexture = new BABYLON.Texture("textures/grass.jpg", this.scene); // Remplacez par le chemin de votre texture de sol
